  ...
  if (obtain_stats == TRUE) {
    ...
    valid_locations <- artis_fishbase_locations %>%
      filter(Status %in% c("native", "Native", "introduced", "endemic", "Endemic") & 
             !is.na(country)) %>%
      mutate(eez_name = countrycode(country, origin = 'country.name', 
...
# REVISED CODE - Now should work
presence_absence_data_linked <- pa_only_artis %>%
  group_by(eez_iso3c) %>%
  group_modify(~create_hierarchical_presence(.x)) %>%
  ungroup()

# Alternative approach if group_modify still causes issues:
# Process each country separately and bind results
create_presence_data_alternative <- function(data) {
  countries <- unique(data$eez_iso3c)
  result_list <- list()
  
  for(i in seq_along(countries)) {
    country_code <- countries[i]
    country_data <- data %>% filter(eez_iso3c == country_code)
    
    processed_data <- create_hierarchical_presence(country_data)
    result_list[[i]] <- processed_data
    
    # Progress indicator
    if(i %% 10 == 0) cat("Processed", i, "of", length(countries), "countries\n")
  }
...
x <- taxa_presence_or_absence(data = "fishbase")
...
presence_absence_data_linked_unlinked <- pa_only_artis %>%
  group_by(eez_iso3c) %>%
  group_modify(~create_hierarchical_presence(.x)) %>%
  ungroup()
...
presence_absence_data_linked_unlinked %>%
  select(-c(isscaap, sciname_joinable_to_fishbase, countries_found, presence_or_absence)) %>%
  relocate(species, .before = genus) %>%
  filter(is.na(source)) %>%
  ungroup() %>%
  filter(str_detect(sciname_hs_modified, " ")) %>%
...
# Look at species that are in ARTIS that are not in fishbase / sealifebase
full_join(sciname,
