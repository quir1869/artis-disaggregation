...
create_hierarchical_presence <- function(country_data, genus_country_lookup = genus_country_lookup) {
  
  # Get all taxa that are definitely present (presence_or_absence == 1)
  present_taxa <- country_data %>%
    filter(presence_or_absence == 1) %>%
    select(sciname_hs_modified, species, genus, subfamily, family, order, class, superclass, phylum, kingdom, countries_found) %>%
    distinct()
  ...
  # Get list of countries that each taxa are present in
  countries_by_genus <- genus_country_lookup
  ...
  country_data %>%
    mutate(
      presence_or_absence_all_taxa = case_when(
        ...
        # Check hierarchical presence - if any more specific level exists and is absent, mark as 0
        # Species level (most specific)
        !is.na(species) & species %in% present_species ~ 1,
        
        # Genus level - only check if species is NA, but if genus is absent = 0
        is.na(species) & !is.na(genus) & genus %in% present_genera ~ 1,
        
        # Subfamily level - only if species and genus are NA
        is.na(species) & is.na(genus) & !is.na(subfamily) & subfamily %in% present_subfamilies ~ 1,
        
        # Family level - only if species, genus, subfamily are NA
        is.na(species) & is.na(genus) & is.na(subfamily) & !is.na(family) & family %in% present_families ~ 1,
        
        # Order level - only if all more specific levels are NA
        is.na(species) & is.na(genus) & is.na(subfamily) & is.na(family) & !is.na(order) & order %in% present_orders ~ 1,
        
        # Class level
        is.na(species) & is.na(genus) & is.na(subfamily) & is.na(family) & is.na(order) & !is.na(class) & class %in% present_classes ~ 1,
        
        # Superclass level
        is.na(species) & is.na(genus) & is.na(subfamily) & is.na(family) & is.na(order) & is.na(class) & !is.na(superclass) & superclass %in% present_superclasses ~ 1,
        
        # Phylum level
        is.na(species) & is.na(genus) & is.na(subfamily) & is.na(family) & is.na(order) & is.na(class) & is.na(superclass) & !is.na(phylum) & phylum %in% present_phyla ~ 1,
        
        # Kingdom level (least specific)
        is.na(species) & is.na(genus) & is.na(subfamily) & is.na(family) & is.na(order) & is.na(class) & is.na(superclass) & is.na(phylum) & !is.na(kingdom) & kingdom %in% present_kingdoms ~ 1,
        
        # Otherwise absent
        TRUE ~ 0
      ),
      countries_found = case_when(
        !is.na(genus) & genus %in% names(countries_by_genus) ~ countries_by_genus[genus],
      TRUE ~ NA_character_
      )) %>%
...
genus_country_lookup <- pa_only_artis %>%
  ...
  group_by(genus) %>%
  summarize(
    countries_for_genus = paste(sort(unique(country)), collapse = " "),
    .groups = "drop"
  ) %>%
...
presence_absence_data <- pa_only_artis %>%
  group_by(eez_iso3c) %>%
  group_modify(~create_hierarchical_presence(.x, genus_country_lookup = genus_country_lookup)) %>%
  relocate(presence_or_absence_all_taxa, .after = presence_or_absence) %>%
  ungroup() %>%
  select(eez_iso3c, sciname_hs_modified, presence_or_absence_all_taxa, countries_found)
...
# Remove more objects
# rm(consumption_all_years, presence_absence_data)f
...
presence_absence_data %>%
  filter(presence_or_absence_all_taxa == 0)
# 
# fread("../data/consumption/sciname.csv") %>%
#   filter(sciname == "loliginidae")
# scinames_excluding_tradeflows %>%
# filter(class == "actinopterygii", eez_iso3c == "DOM")
# 
#  country("Lindbergichthys mizops")
# 
# scinames_excluding_tradeflows %>%
#   filter(sciname_hs_modified == "lindbergichthys mizops")

# sciname %>%
#   filter(if_any(everything(), ~ str_detect(as.character(.x), regex("bathyraja irrasa", ignore_case = TRUE))))
# 
# country("Bathyraja irrasa", server = "sealifebase")
# 
# setdiff(sciname$sciname[str_detect(sciname$sciname, " ")],fb_slb$ScientificName)[setdiff(sciname$sciname[str_detect(sciname$sciname, " ")],fb_slb$ScientificName) == "bathyraja irrasa"]
# 
# fb_slb %>%
#   filter(if_any(everything(), ~ str_detect(as.character(.x), regex("bathyraja irrasa", ignore_case = TRUE))))

test_data <- pa_only_artis %>%
    filter(presence_or_absence == 1) %>%
    select(sciname_hs_modified, species, genus, subfamily, family, order, class, superclass, phylum, kingdom, countries_found) %>%
    distinct()

paste(unique(strsplit(paste((test_data %>%
  filter(class == "actinopterygii"))$countries_found, collapse = " "), " ")[[1]]), collapse = " ")

  
  
paste("USA CHN", "CAD BOS", sep = " ")


pa_only_artis %>%
    filter(presence_or_absence == 1, !is.na(genus)) %>% 
    # split the space-separated string into one row per country
    mutate(country = strsplit(countries_found, " ")) %>% 
    unnest(country) %>% 
    distinct(genus, country) %>% 
    # re-collapse for each genus
    group_by(genus) %>% 
    summarize(countries_for_genus = paste(sort(unique(country)), collapse = " "), 
              .groups = "drop")


left_join(presence_absence_data, sciname, by = c("sciname_hs_modified" = "sciname")) %>%
  filter(!is.na(genus),
         !str_detect(sciname_hs_modified, " "))

presence_absence_data %>%
  filter(presence_or_absence_all_taxa == 0)

left_join(presence_absence_data, sciname, by = c("sciname_hs_modified" = "sciname")) %>%
  filter(str_detect(sciname_hs_modified, "cantherhines"))

presence_absence_data %>%
  count(presence_or_absence_all_taxa)

pa_only_artis[apply(pa_only_artis, 1, function(row) any(grepl("murex", row, ignore.case = TRUE))), ]
pa_only_artis[apply(pa_only_artis, 1, function(row) any(grepl("cantherhines", row, ignore.case = TRUE))), ]
...
any(str_detect(present_genera, "cantherhines"))
```
